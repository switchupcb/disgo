...
func generateComment(function *models.Function) string {
	return "// SendEvent sends an Opcode " + function.Options.Custom["opcode"][0] + " " + function.Name + " event to the Discord Voice Server."
}

// generateSignature generates a function's signature.
func generateSignature(function *models.Function) string {
	return "func (c " + function.From[0].Field.FullDefinition() + ") SendEvent(bot *Client, session *VoiceSession) (" +
...
func generateBody(function *models.Function) string {
	...
	flag := "FlagVoiceSendEventName" + function.Name

	// rename VoiceIdentify to Identify.
	if name == "VoiceIdentify" {
		name = "Identify"
	}

	// rename VoiceHeartbeat to Heartbeat.
	if name == "VoiceHeartbeat" {
		name = "Heartbeat"
	}

	// rename VoiceResume to Resume.
	if name == "VoiceResume" {
		name = "Resume"
	}

	body.WriteString("if err := writeEventVoice(bot, session, FlagVoiceOpcode" + name + ", " +
		flag + ", c); err != nil {\n")
